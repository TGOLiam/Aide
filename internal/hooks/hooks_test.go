...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["AIDE_EVENT"])
	require.Equal(t, "bash", envMap["AIDE_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["AIDE_SESSION_ID"])
	require.Equal(t, "/work", envMap["AIDE_CWD"])
	require.Equal(t, "/project", envMap["AIDE_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["AIDE_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["AIDE_TOOL_INPUT_FILE_PATH"])
	...
	require.Equal(t, "aide", envMap["AGENT"])
	require.Equal(t, "aide", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$AIDE_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("aide format still works", func(t *testing.T) {
		t.Parallel()
