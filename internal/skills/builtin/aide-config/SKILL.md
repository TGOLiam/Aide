---
name: aide-config
description: Use when the user needs help configuring aide — working with aide.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing aide behavior.
---

# aide Configuration

aide uses JSON configuration files with the following priority (highest to lowest):

1. `.aide.json` (project-local, hidden)
2. `aide.json` (project-local)
3. `$XDG_CONFIG_HOME/aide/aide.json` or `$HOME/.config/aide/aide.json` (global)

## Basic Structure

```json
{
  "$schema": "https://charm.land/aide.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
aide runs selected string fields through an embedded bash-compatible
...
`aide.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch aide in a directory whose `aide.json` you haven't
reviewed.
...
> `.agents/skills`, `.aide/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".aide/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".aide/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `aide_EVENT` | Event name (e.g. `PreToolUse`) |
| `aide_TOOL_NAME` | Name of the tool being called |
| `aide_SESSION_ID` | Current session ID |
| `aide_CWD` | Current working directory |
| `aide_PROJECT_DIR` | Project root directory |
| `aide_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `aide_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
aide also supports the Claude Code hook output format:
...
- `aide_GLOBAL_CONFIG` - Override global config location
- `aide_GLOBAL_DATA` - Override data directory location
- `aide_SKILLS_DIR` - Override default skills directory
