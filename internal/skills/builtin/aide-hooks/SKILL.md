---
name: aide-hooks
description: Use when the user wants to add, write, debug, or configure a aide hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in aide.json.
---

# aide Hooks

Hooks are user-defined commands in `aide.json` that fire at specific points
...
| `aide_TOOL_NAME`            | Tool being called (e.g. `bash`)          |
| `aide_SESSION_ID`           | Current session ID                       |
| `aide_CWD`                  | Working directory                        |
| `aide_PROJECT_DIR`          | Project root directory                   |
| `aide_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command      |
| `aide_TOOL_INPUT_FILE_PATH` | For file tools: the target file path     |
...
if echo "$aide_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
if [[ "$aide_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
3. Add the entry under `hooks.PreToolUse` in `aide.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check aide logs.
...
aide also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: aide treats `updated_input` as shallow-merge, Claude Code replaces.
