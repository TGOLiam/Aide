...
// event into a closure nobody reads, and subscribers (`aide run`)
...
// publish a RunComplete for its RunID, hanging a `aide run` caller that
...
// than internal bookkeeping, which is the contract a `aide run` caller
...
// bookkeeping slice, not the published event a `aide run` caller awaits.
