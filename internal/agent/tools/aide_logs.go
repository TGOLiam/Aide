...
var aideLogsDescriptionTmpl []byte

var aideLogsDescriptionTpl = template.Must(
	template.New("aideLogsDescription").
		Parse(string(aideLogsDescriptionTmpl)),
)

type aideLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func aideLogsDescription() string {
	return renderTemplate(aideLogsDescriptionTpl, aideLogsDescriptionData{
...
func NewCrushLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		CrushLogsToolName,
		aideLogsDescription(),
		func(ctx context.Context, params AideLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runAideLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runAideLogs reads and formats the last N log entries from the given file.
func runAideLogs(logFile string, params AideLogsParams) string {
