...
// already in use by another aide process.
var ErrDataDirLocked = errors.New("data directory already in use by another aide process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to aide.db so users can `ls` and find it.
const dataDirLockFile = "aide.lock"
...
// {dataDir}/aide.lock. If the lock is already held by another
...
// AIDE_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("AIDE_SKIP_DATADIR_LOCK"))
	return v
}
