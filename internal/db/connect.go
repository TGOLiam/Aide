...
// aide process from racing the storage.
...
// workspace-bootstrap path opts in. AIDE_SKIP_DATADIR_LOCK still
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	// aide process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second aide against the
