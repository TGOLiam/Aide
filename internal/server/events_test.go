...
// are what non-interactive clients (e.g. `aide run`) rely on to
...
// through the SSE envelope. Without these correlators, `aide run`
...
// "agent failed" (returns non-zero from `aide run`) from "agent
