...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .aideignore file that ignores .log files
	require.NoError(t, os.WriteFile(".aideignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .aideignore file
	crushignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".aideignore"), []byte(crushignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .aideignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .aideignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .aideignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".aideignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .aideignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .aideignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .aideignore")
