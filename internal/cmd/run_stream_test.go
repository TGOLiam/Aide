...
// with reason=tool_use and used to terminate `aide run` early via
...
// subscribers (e.g. a TUI watching workspace events while `aide
...
// non-nil error from `aide run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `aide run` waits) exits cleanly rather
...
// queue scenario: `aide run --continue` attaches to a session
