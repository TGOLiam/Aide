...
import (
	...
	"github.com/liamb/opencode/aide/internal/format"
...
curl https://charm.land | aide run "Summarize this website"

# Read from a file
aide run "What is this code doing?" <<< prrr.go

# Redirect output to a file
aide run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
aide run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
aide run --verbose "Generate a README for this project"

# Continue a previous session
aide run --session {session-id} "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'aide' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `aide run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `aide run` exited
...
// prefix returned by aide session list.
