...
import (
	...
	"github.com/liamb/aide/internal/discover"
...
func PushPopCrushEnv() func() {
	...
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "AIDE_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "AIDE_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("AIDE_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("AIDE_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("AIDE_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated aide.json placed above the project is never picked
...
func GlobalConfig() string {
	if crushGlobal := os.Getenv("AIDE_GLOBAL_CONFIG"); crushGlobal != "" {
...
func GlobalCacheDir() string {
	if crushCache := os.Getenv("AIDE_CACHE_DIR"); crushCache != "" {
		return crushCache
	}
...
func GlobalConfigData() string {
	if crushData := os.Getenv("AIDE_GLOBAL_DATA"); crushData != "" {
	...
	// for windows, it should be in `%LOCALAPPDATA%/aide/`
	// for linux and macOS, it should be in `$HOME/.local/share/aide/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
func GlobalSkillsDirs() []string {
	if crushSkills := os.Getenv("AIDE_SKILLS_DIR"); crushSkills != "" {
	...
	// On Windows, also load from app data on top of `$HOME/.config/aide`.
