---
name: shell-builtins
description: Use when creating a new shell builtin command for aide (internal/shell/), editing an existing one, or when the user needs to understand how commands are intercepted in aide's embedded shell.
---

# Shell Builtins

aide's shell (`internal/shell/`) uses `mvdan.cc/sh/v3` for POSIX shell
