...
name: builtin-skills
description:
  Use when creating a new builtin skill for aide, editing an existing builtin
...
aide embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `aide://skills/` prefix (e.g., `aide://skills/jq/SKILL.md`).
- The View tool resolves `aide://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `aide-config` | `builtin/aide-config/` | aide configuration help                   |
| `aide-hooks`  | `builtin/aide-hooks/`  | Authoring, configuring and debugging hooks |
