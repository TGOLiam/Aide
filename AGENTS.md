...
aide is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/charmbracelet/aide`.
...
internal/
  ...
  config/
    ...
    load.go                        aide.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (aide + Claude Code compat)
...
- **Context files**: aide reads AGENTS.md, aide.md, CLAUDE.md, GEMINI.md
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `aide.json` that fire before
